...
app.get("/api/_debug/scenario/:id/inspect", requireAdmin, async (req, res) => {
  ...
  app.get("/api/_debug/effects", requireAdmin, async (_req, res) => {
    ...
    if (auErr) return res.status(500).json({ status: "ERROR", where: "users", message: auErr.message });
    ...
    if (appErr) return res.status(500).json({ status: "ERROR", where: "applications", message: appErr.message });
